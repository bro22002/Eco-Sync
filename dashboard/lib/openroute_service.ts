...
const API_KEY_AVAILABLE = !!OPENROUTE_API_KEY && OPENROUTE_API_KEY.length > 0
...
export async function geocodeLocation(location: string): Promise<{ lat: number; lon: number }> {
  ...
  // If API key is not available, skip OpenRoute and let caller handle fallback
  if (!API_KEY_AVAILABLE) {
    throw new Error(`OpenRoute API key not configured. Falling back to default distances.`)
  }
