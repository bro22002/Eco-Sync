...
class Tree {
  ...
  constructor(carbonScore: number, transportType: 'air' | 'sea' | 'land', maxDepth: number = 10) {
   ...
   */
  draw(p: p5, x: number, y: number, angle: number, depth: number = 0, branchLength?: number): void {
...
export default function P5TreeWrapper({ trees, overallCarbonScore }: P5TreeWrapperProps) {
      ...
      p.draw = () => {
        ...
        if (treeInstances.length !== trees.length) {
          treeInstances = trees.map(
            (treeData) => new Tree(treeData.carbonScore, treeData.transportType)
          )
        }
        ...
        if (trees.length >= 3) {
          ...
          p.bezier(x1, y1, (x1 + x2) / 2, y1 - 100, (x1 + x2) / 2, y1 - 100, x2, y1)
        }

        // Draw carbon score indicator
        drawCarbonIndicator(p, overallCarbonScore)
      }

      function drawCarbonIndicator(p: p5, score: number) {
        ...
        p.fill(200, 200, 200)
        p.textSize(10)
        p.text('Carbon Score', centerX, topY + 35)
      }
